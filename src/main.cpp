...
int main()
{
    ...
    const double time_final = 2.0;  // terminal simulation time
    const double time_step  = 0.01; // simulation time step

    const Vector3d I_vec           = {0.1, 0.1, 0.5};                               // (3x1) nominal moment of inertia
    const Vector6d Gamma_omega_vec = {0.1, 0.1, 0.1, 0.0, 0.0, 0.0};                // (6x1) vectorized adaptive gain for Y'omega_a
    const Vector6d Gamma_theta_vec = {0.05, 0.05, 0.05, 0.0, 0.0, 0.0};             // (6x1) vectorized adaptive gain for tilde_theta
    ...
    const Vector6d theta_star      = {1 / I_vec(0) - 1.0, 1 / I_vec(1) - 1.0, 1 / I_vec(2) - 1.0, (I_vec(1) - I_vec(2)) / I_vec(0), (I_vec(2) - I_vec(0)) / I_vec(1), (I_vec(1) - I_vec(0)) / I_vec(2)};
    Param para;
    ...
    const Vector6d theta_init     = {9.0, 9.0, 1.0, -4.0, 4.0, 0.0}; // (6x1) initial estimation values
    ...
    cout.setf(ios::right);
    cout << theta_star.transpose() << endl;
    while (t <= time_final)
    {
